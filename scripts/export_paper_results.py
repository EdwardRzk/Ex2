...
OPTIONAL_COMPATIBLE = ("Cross-Candidate MambaNVP",)
...
def add_final(records: list[dict[str, Any]], method: str, source: str, macro: Mapping[str, Any], per_dataset: Mapping[str, Any], nvp_macro: float, nvp_datasets: Mapping[str, Any], *, source_method: str | None = None, regret: float | None = None, recovery: float | None = None, top1: float | None = None, top5: float | None = None, median_delta: float | None = None, positive: int | None = None, negative: int | None = None, leave_llvm: float | None = None) -> None:
    ...
    for dataset, values in sorted(per_dataset.items()):
        method_values=values[source_method or method] if (source_method or method) in values else values
...
def export(root: Path, output: Path) -> None:
    ...
    direct_combined=direct_f["combined_comparison"]; add_final(records,"Direct MambaNVP",sources["Direct-final"],direct_combined["dataset_macro"]["MambaNVP"],direct_combined["per_dataset"],nvp_f,nvp_datasets,source_method="MambaNVP",regret=direct_f["policy45_regret"]["MambaNVP"]["mean_3seed"],recovery=direct_f["mamba_nvp_oracle_recovery"])
    anchored_combined=anchored_f["combined_comparison"]; a=anchored_f["gated_calibrated_mambanvp"]; add_final(records,"Anchored MambaNVP",sources["Anchored-final"],anchored_combined["dataset_macro"]["GatedCalibratedMambaNVP"],anchored_combined["per_dataset"],nvp_f,nvp_datasets,source_method="GatedCalibratedMambaNVP",regret=a["policy45_regret"]["mean_3seed"],recovery=a["oracle_recovery"],top1=a["top1_accuracy_3seed"])
    ...
    cross_combined=cross_f["combined_comparison"]; add_final(records,"Cross-Candidate MambaNVP",sources["Cross-final"],cross_combined["dataset_macro"]["CrossCandidateMambaNVP"],cross_combined["per_dataset"],nvp_f,nvp_datasets,source_method="CrossCandidateMambaNVP",regret=cross_f["policy45_regret"]["CrossCandidateMambaNVP"]["mean_3seed"],recovery=cross_f["oracle_recovery"]["CrossCandidateMambaNVP"])
    ...
    headline={method:{split:next(x["MeanOverOz"] for x in records if x["method"]==method and x["split"]==split and x["record_type"]=="dataset_macro_3seed") for split in ("validation","final/OOD")} for method in MAIN+OPTIONAL_COMPATIBLE}
    output.mkdir(parents=True); metadata={"task":"frozen report recovery only","offline_evaluator_rerun_required":False,"compiler_gym_initialized":False,"llvm_execution":False,"objecttext_observation":False,"primary_reports_fully_recoverable":list(MAIN+OPTIONAL_COMPATIBLE),"methods_with_complete_secondary_metrics":["PA-MambaNVP"],"methods_partial_metrics":{"NVP":"top1/top5 unavailable; final policy45 regret unavailable in aggregate","MLP":"top1/top5 unavailable; final policy45 regret unavailable in aggregate","LSTM":"top1/top5 unavailable; final policy45 regret unavailable in aggregate","Transformer":"top1/top5 unavailable; final policy45 regret unavailable in aggregate","Mamba":"top1/top5 unavailable; final policy45 regret unavailable in aggregate","Direct MambaNVP":"top1/top5 unavailable in formal reports","Anchored MambaNVP":"top5 unavailable in formal reports","Cross-Candidate MambaNVP":"top1/top5 unavailable in formal reports"},"optional_cross_candidate":{"status":"included in canonical JSON/CSV but excluded from eight-row main table","reason":"compatible frozen formal reports are available; requested main table is restricted to eight methods"},"sources":sources,"records":records,"headline_values":headline}
    (output/"paper_results.json").write_text(json.dumps(metadata,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    fields=list(records[0]);
    with (output/"paper_results.csv").open("w",newline="",encoding="utf-8") as handle: writer=csv.DictWriter(handle,fieldnames=fields,lineterminator="\n"); writer.writeheader(); writer.writerows(records)
    with (output/"main_results_table.csv").open("w",newline="",encoding="utf-8") as handle:
        fields2=["method","validation_mean_over_oz","final_ood_mean_over_oz","validation_delta_vs_nvp","final_ood_delta_vs_nvp","paper_display"]; writer=csv.DictWriter(handle,fieldnames=fields2,lineterminator="\n"); writer.writeheader()
        for method in MAIN:
