...
def correction_summary(rows: Sequence[Mapping[str, Any]], baseline: str, corrected: str) -> dict[str, Any]:
    ...
    return {"overall": correction_groups(output), "per_dataset": {dataset: correction_groups(group) for dataset, group in grouped(output, ("dataset_id",)).items() for dataset in [dataset[0]]}}
...
def disagreement_summary(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {"overall": collapse(output), "per_dataset": {key[0]: collapse(group) for key, group in grouped(output, ("dataset_id",)).items()}}
